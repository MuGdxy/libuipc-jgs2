...
template <typename T>
bool vector_contains(vector<T> vec, T val)
{
    bool contains = false;
...
template <typename T>
void print_vector(vector<T>& vec, string sep)
{
    for(int i = 0; i < vec.size(); i++)
    {
        cout << i << ":" << vec[i] << sep;
...
class StopWatch
{
  public:
    ...
    void tic()
    {
        if(inCycle)
        {
            cerr << "** WARNING: StopWatch::tic was called again before toc was called!"
                 << endl;
        }
    ...
    double toc()
    {
        if(!inCycle)
        {
            cerr << "** WARNING: StopWatch::toc called before tic was called! Returning bogus time!"
                 << endl;
            return -1.0;
        }
    ...
    double getTotalSecs() { return clock2secs(totalTics); }

    void reset()
    {
        if(inCycle)
        {
            cerr << "** WARNING: StopWatch::reset was called between a tic/toc cycle!"
                 << endl;
        }
        totalTics = 0;
    }
...
// for libconfig++
#define LOAD_CFG_VAR(cfg, var, defval)                                         \
    {                                                                          \
        if((cfg).exists(#var))                                                 \
            cfg.lookupValue(#var, var);                                        \
        else                                                                   \
            var = (defval);                                                    \
    }
