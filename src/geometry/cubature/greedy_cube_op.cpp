...
class NnlsSolver
{
  public:
    ...
    bool solve(double* A_colmajor, int numCols, VECTOR& b, VECTOR& x, double& residualNormOut)
    {
        ...
        // do it
        nnls(_workA, _rows, _rows, numCols, _workB, x.data(), &residualNormOut, _workW, _workZ, _workIndex, &_workMode);
...
{
#ifdef USE_CBLAS
    cblas_dgemv(CblasColMajor,  // important
                CblasNoTrans,
                m,
                n,
                1.0,
...
{
#ifdef USE_CBLAS
    cblas_dgemv(CblasRowMajor,  // important
                CblasNoTrans,
                m,
                n,
                1.0,
...
template <typename T>
int find_max(T* array, int size, T& maxVal)
{
    int maxIdx = 0;
    maxVal     = array[0];
...
void GreedyCubop::gatherRandomUnusedPoints(vector<int>& usedPoints,
...
void GreedyCubop::evalPointColumn(int           pointId,
...
static void normalizeTrainingForces(int r, VECTOR& forces, vector<Real>& inverseForceNorms)
{
    ...
    for(int t = 0; t < T; t++)
    {
        ...
        {
            cout << "R-mag-sq of training set " << t << " was near zero: " << bblockSqrSum
                 << ". doing no scaling." << endl;
...
void sample_range(int start, int end, int numSamples, vector<int>& samplesOut, std::random_device& rd)
{
    fill_range(samplesOut, start, end);
    std::mt19937 gen(rd());
    std::shuffle(samplesOut.begin(), samplesOut.end(), gen);
    samplesOut.resize(numSamples);
}

// analogous to matlab: out = from( subsetIndices )
template <typename T>
void collect_subset(vector<T>& from, vector<int>& subsetIndices, vector<T>& out)
{
 ...
 */
int GreedyCubop::findMostFitPoint(std::vector<int>& candidatePoints,
                                  VECTOR&           residual,
                                  TrainingSet&      trainingSet,
                                  vector<Real>&     inverseForceNorms)
{
...
void GreedyCubop::selectCandidatePoints(vector<int>& usedPoints,
                                        int          numCandidates,
                                        vector<int>& candidatesOut)
{
...
void GreedyCubop::run(TrainingSet& trainingSet,
...
)
{
    ...
    while(relErr > relErrTol && selectedPoints.size() < maxNumPoints
          && selectedPoints.size() < numTotalPoints())
    {
        ...
        copy(inverseForceNorms.begin(),
             inverseForceNorms.end(),
             inverseForceNormsSubset.begin());
        ...
        for(int i = 0; i < itersPerFullNNLS && selectedPoints.size() < maxNumPoints; i++)
        {
            ...
            int bestPointId =
                findMostFitPoint(candPointIds, subsetResidual, trainingSubset, inverseForceNormsSubset);
            ...
            sample_range(0, T, numSamplesPerSubtrain, subsetIndices, _rd);
            ...
            bool converged = subNNLS.solveRowMajor(
                subsetA.data(), subsetA.cols(), selectedPoints.size(), subsetB, w, residualNorm);

            subsetResidual.resizeAndWipe(numSamplesPerSubtrain * _r);
            calcRowMajorResidual(
                subsetA.data(), subsetA.cols(), numSamplesPerSubtrain, _r, w, subsetB, subsetResidual);
        ...
        w.resizeAndWipe(selectedPoints.size());
        bool converged =
            nnlser.solveRowMajor(A.data(), A.cols(), selectedPoints.size(), b, w, residualNorm);
        ...
        {
            cerr << "[ERROR] ** NNLS did not converge for selectedPoints.size() = "
                 << selectedPoints.size() << endl;
        }
    }
}
...
{
    cout << "n = " << selectedPoints.size() << " \t relerr = " << (relErr * 100)
         << "%" << endl;
}
...
void GreedyCubop::runNQP(TrainingSet& trainingSet,  // list of q vectors
                         VECTOR& trainingForces,  // r*T vector of ground-truth forces. 'b' in the paper.
                         Real relErrTol,    // TOL
                         int maxNumPoints,  // some sane limit, for overnight runs
                         int  numCandsPerIter,  // |C|
