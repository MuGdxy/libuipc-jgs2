...
void SteveLib::bin_read(istream& s, int& dst)
{
    s.read((char*)&dst, sizeof(int));
}
void SteveLib::bin_read(istream& s, double& dst)
{
    s.read((char*)&dst, sizeof(double));
}

void SteveLib::bin_write(ostream& s, const int& dst)
{
    s.write((char*)&dst, sizeof(int));
}
void SteveLib::bin_write(ostream& s, const double& dst)
{
    s.write((char*)&dst, sizeof(double));
}
...
string SteveLib::get_split_part(const string& s, const string& splitters, int i)
{
    ...
    while(i != 0)
    {
        ...
        {
            cerr << "couldn't find any occurrance of '" << splitters << "' in '"
                 << s << "'" << endl;
            return string("");
        }
