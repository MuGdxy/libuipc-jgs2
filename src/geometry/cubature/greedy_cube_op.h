...
class GreedyCubop
{
  public:
    ...
    void run(TrainingSet& trainingSet,  // list of q vectors
    ...
    void runNQP(TrainingSet& trainingSet,  // list of q vectors
                VECTOR& trainingForces,  // r*T vector of ground-truth forces. 'b' in the paper.
                Real relErrTol,        // TOL
                int  maxNumPoints,     // some sane limit, for overnight runs
                int  numCandsPerIter,  // |C|
	 ...
	 */
    void randomSeed(int seed) { _rand.seed(seed); }

  private:
    MERSENNETWISTER    _rand;
    std::random_device _rd;  // For random number generation
    ...
    int findMostFitPoint(std::vector<int>& candidatePoints,
                         VECTOR&           residual,
                         TrainingSet&      trainingSet,
                         vector<Real>&     inverseForceNorms);

    void gatherRandomUnusedPoints(std::vector<int>& usedPoints,
                                  std::vector<int>& candidatesOut,
                                  int               maxCands);
  ...
  protected:
	 ...
	 *	pointId - an index in [0, getNumTotalPoints()). Implementations must map this index deterministically to a cubature point.
	 */
    virtual void evalPointColumn(int           pointId,
                                 TrainingSet&  trainingSet,
                                 vector<Real>& inverseForceNorms,
                                 VECTOR&       columnOut);
	 ...
	 */
    virtual void selectCandidatePoints(std::vector<int>& usedPoints,
                                       int               numCandidates,
                                       std::vector<int>& candidatesOut);
};
